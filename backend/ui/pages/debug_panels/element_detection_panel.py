...
class ElementDetectionPanel(QGroupBox):
    ...
    def _on_add_condition(self) -> None:
        ...
        item = QListWidgetItem(f"{template_key}  (阈值: {threshold:.2f})")
        item.setData(Qt.ItemDataRole.UserRole, (template_key, threshold))
    ...
    def _on_detect(self) -> None:
        ...
        try:
            ...
            for template_key, threshold in conditions:
                ...
                best_score, best_loc, best_scale, best_size = \
                    self._multi_scale_match(screen_gray, template)
                ...
                tw, th = best_size
                result.draw_rect(
                    best_loc[0], best_loc[1], tw, th,
                    color=color, thickness=3,
                    label=f"{template_key} {best_score:.2f}",
                )
                detail_parts.append(
                    f"{'✓' if passed else '✗'}{template_key}: {best_score:.3f}@{best_scale:.2f}x"
                )
            ...
            if all_passed:
                self._label_result.setText(
                    f"✓ 全部通过 ({passed_count}/{total}) | "
                    + " | ".join(detail_parts)
                )
            else:
                self._label_result.setText(
                    f"✗ 未通过 ({passed_count}/{total}) | "
                    + " | ".join(detail_parts)
                )
